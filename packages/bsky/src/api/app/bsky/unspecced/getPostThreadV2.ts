...
export default function (server: Server, ctx: AppContext) {
  ...
  server.add(app.bsky.unspecced.getPostThreadV2, {
    ...
    handler: async ({ params, auth, req }) => {
      ...
      // Community posts: synthesized from community_post rather than run
      // through the standard pipeline. Shared with the community thread
      // endpoint, which serves the same shape for space URIs.
